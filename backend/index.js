...
// Database Conntection With Mongidb

mongoose.connect(
  "mongodb+srv://shpalovartem:otsxUFp5FklZ7GAk@cluster1.tg8gq0b.mongodb.net/?appName=Cluster1",
);
...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res.status(400).json({
      success: false,
      errors: "existing user found with some email address",
    });
  }
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    res.status(401).send({ errors: "Please authtenticate using validt oken" });
  } else {
    try {
